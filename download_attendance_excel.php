<?php
// =======================================================
// Attendance Excel Export Script (Fixed)
// =======================================================

// 1️⃣ Absolutely no space/newline before "<?php"
session_start();
if (!isset($_SESSION['userID'])) {
    die("Access Denied.");
}

// Disable PHP error display to prevent header corruption
...
// Dept office can see all sections
...
// =======================================================
$subjects = [];
$subQ = "SELECT subject_code, subject_name 
         FROM subjects 
         WHERE dept='" . $conn->real_escape_string($deptFilter) . "'
...
if ($month !== "All") {
    $attQuery .= " AND month='" . $conn->real_escape_string($month) . "'";
}
...
// FETCH STUDENTS
// =======================================================
$stuQuery = "SELECT studentId, section 
             FROM userstudent 
             WHERE dept='" . $conn->real_escape_string($dept) . "' 
               AND year='" . $conn->real_escape_string($year) . "'";
if ($sectionFilter) {
    $stuQuery .= " AND section='" . $conn->real_escape_string($sectionFilter) . "'";
}
...
$fileName = empty($sectionSafe)
    ? "{$year}{$deptCode}_sem{$semester}_AY{$academic_year}_{$examSafe}_attendance.xlsx"
    : "{$year}{$deptCode}_section{$sectionSafe}_sem{$semester}_AY{$academic_year}_{$examSafe}_attendance.xlsx";
