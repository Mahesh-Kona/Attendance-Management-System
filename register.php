...
$user = "root";      
$pass = "";          
$db   = "attendance_management_system";  // your DB name
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_faculty_excel'])) {
    if (isset($_FILES['faculty_excel']) && $_FILES['faculty_excel']['error'] == 0) {
        ...
        try {
            ...
            echo "<script>alert('Faculty Records Uploaded Successfully!'); window.location.href='dept_office_dashboard.php';</script>";
        } catch (Exception $e) {
    ...
    if (isset($_FILES['student_excel']) && $_FILES['student_excel']['error'] == 0) {
        ...
        try {
            ...
            for ($i = 1; $i < count($rows); $i++) {
                ...
                $question    = trim($rows[$i][6]);
                $answer      = strtolower(trim($rows[$i][7]));
                $section     = trim($rows[$i][8]);
                $year        = trim($rows[$i][9]);
                $academicYear= trim($rows[$i][10]);

                if (!empty($studentID)) {
                    $sql = "INSERT INTO userstudent 
                            (studentID, studentName, password, contact, dept, security_question, security_answer, section, year, academic_year) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            ON DUPLICATE KEY UPDATE 
                    ...
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("ssssssssss", $studentID, $studentName, $password, $contact, $deptExcel, $question, $answer, $section, $year, $academicYear);
                    $stmt->execute();
                    $stmt->close();
                }
            }
            echo "<script>alert('Student Records Uploaded Successfully!'); window.location.href='dept_office_dashboard.php';</script>";
        } catch (Exception $e) {
            echo "<script>alert('Error Reading Excel: " . $e->getMessage() . "');</script>";
        }
    } else {
    ...
    <?php if ($role == 'faculty') { ?>
        ...
        <div class="card shadow p-4 mx-auto mt-4" style="max-width: 500px;">
            ...
            <form method="POST" enctype="multipart/form-data" action="">
                ...
                <button type="submit" class="btn btn-primary w-100">Upload Faculty Data</button>
            </form>
        </div>
    <?php } ?>
